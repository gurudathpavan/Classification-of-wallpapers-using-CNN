...
checkpointDir = 'modelCheckpointsWide_BC';
...
layers = [
    ...
    convolution2dLayer(5,60,'Padding',[2 2],'Stride', [1,1]);  % convolution layer with 60, 5x5 filters
...
saveas(error_plot, ['BC_Wide_Error_plot_',num2str(numEpochs),'.png']);
...
filename = ['BC_Training_Confusion_Mat_Wide_', num2str(numEpochs), '.xlsx'];
xlswrite(filename, con_mat_train,'Sheet1','A1');

filename = ['BC_Training_Classification_Mat_Wide_', num2str(numEpochs), '.xlsx'];
...
filename = ['BC_Validation_Confusion_Mat_Wide_', num2str(numEpochs), '.xlsx'];
xlswrite(filename, con_mat_val,'Sheet1','A1');

filename = ['BC_Validation_Classification_Mat_Wide_', num2str(numEpochs), '.xlsx'];
...
filename = ['BC_Testing_Confusion_Mat_Wide_', num2str(numEpochs), '.xlsx'];
xlswrite(filename, con_mat_test,'Sheet1','A1');

filename = ['BC_Testing_Classification_Mat_Wide_', num2str(numEpochs), '.xlsx'];
