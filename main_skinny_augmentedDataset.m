...
layers = [
    ...
    convolution2dLayer(5,40,'Padding',[2 2],'Stride', [1,1]);  % convolution layer with 40, 3x3 filters
    reluLayer();  % ReLU layer
    maxPooling2dLayer(2,'Stride',2); % Max pooling layer
...
saveas(error_plot, ['Skinny_Error_plot_',num2str(numEpochs),'.png']);
...
filename = ['Training_Confusion_Mat_Skinny_', num2str(numEpochs), '.xlsx'];
xlswrite(filename, con_mat_train,'Sheet1','A1');

filename = ['Training_Classification_Mat_Skinny_', num2str(numEpochs), '.xlsx'];
xlswrite(filename, class_mat_train,'Sheet1','A1');

%% Test on the validation data and Test Data
...
filename = ['Validation_Confusion_Mat_Skinny_', num2str(numEpochs), '.xlsx'];
xlswrite(filename, con_mat_val,'Sheet1','A1');

filename = ['Validation_Classification_Mat_Skinny_', num2str(numEpochs), '.xlsx'];
...
filename = ['Testing_Confusion_Mat_Skinny_', num2str(numEpochs), '.xlsx'];
xlswrite(filename, con_mat_test,'Sheet1','A1');

filename = ['Testing_Classification_Mat_Skinny_', num2str(numEpochs), '.xlsx'];
xlswrite(filename, class_mat_test,'Sheet1','A1');

%% Visualization using DeepDreamImage

ddI = net1.Layers(2,1).NumFilters;

Starter_conv1_viz = deepDreamImage(net1,2,ddI);

figure;
imshow(Starter_conv1_viz);
title('First Convolutional Layer of Starter Network');
